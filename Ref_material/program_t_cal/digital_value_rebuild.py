...
def digital_value_rebuild():
    ###############################################
    # parameter definition
    ###############################################
    print("Number of processors: ", mp.cpu_count())
    image_resolution = [30, 30]                                               # [pixel] [100 * 100]
    diameter_ratio = 0.9                                                        # adjust the visualisation of data_field

    temperature_center = 1350                                                   # temperature of the center_area
    temperature_background = 500                                                 # set background temperature to 50K as black body
    temperature_distribution = 'linear'                                         # gaussian / linear / sigmoid
    emissivity_set = 0                                                        # which data set is used
    ...
    T_address = os.path.join('camera_parameter', 'FIFO-Lens_tr.xls')
    cam_param = read_cam(QE_address, T_address)
    ...
    for channel in cam_efficiency.keys():
        ...
        def radiation_integration(temperature, emissivity_data, cam_efficiency_data):
            def radiation(wl):
                return black_body_radiation(temperature, wl) * emissivity(temperature, wl, emissivity_data) * camera_efficiency(wl, cam_efficiency_data)

            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                radiation_rec = quad(radiation, 500 * (10**-9), 1000 * (10**-9), limit=5)[0] * 200      # 200 means the calibration factor and shutter time
        ...
        final_results[channel] = np.array(digital_value).reshape(image_resolution).astype(float)

    # calculate emissivity_map
    emi_field = emissivity_map(t_field, interp_emissivity)
...
#### bb
def emissivity(temperature, wavelength, emi_function):
    # efunc = emi_function(wavelength*(10**9))/4+0.7
    emi = 0.05
    return emi

# def emissivity(temperature, wavelength, emi_function):
#     # efunc = emi_function(wavelength*(10**9))/4+0.7
#     efunc = emi_function(wavelength * (10 ** 9))
#     emi = ( efunc * 0.1 * (temperature - 1500) / 500 + efunc)
#     if temperature>1500:
#         emi = emi*0.1
#     return emi
...
def emissivity_map(t_field, emi_function):
    row, colum = t_field.shape
    emi_field = np.zeros(t_field.shape)
    wavelength = np.arange(500 * (10**-9), 1001 * (10**-9))
    for i in range(row):
        for j in range(colum):
            emi_field[i, j] = np.average([emissivity(t_field[i, j], wl, emi_function) for wl in wavelength])
    return emi_field
