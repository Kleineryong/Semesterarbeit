...
def save_result(t_raw, emi_raw, t_ori, emi_model_name, save_dir):
    ...
    for i in range(len(dir_save_list)):
        ...
        plt.figure(figsize=(7, 6), dpi=400)
        if dir_save_list[i] == '5':
            v_min = 1500
        else:
            v_min = 1000
        plt.imshow(np.abs(t_ori[dir_list[i]]), cmap='inferno', vmin=v_min, vmax=1900)
        cbar = plt.colorbar()
        plt.xlabel('X_position', fontsize=20)
        plt.ylabel('Y_position', fontsize=20)
        plt.title('Cal. temperature', fontsize=20)
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, dir_save_list[i], emi_model_name, 'T_cal.jpg'))
        plt.clf()
        plt.close()
...
def read_result(result_dir):
    ...
    for result in dir_list:
        ...
        t_ori[result] = np.array(pd.read_excel(os.path.join(result_dir, result, 't_cal_1900.xlsx'), header=None))
