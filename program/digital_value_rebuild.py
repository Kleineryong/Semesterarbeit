...
def digital_value_rebuild():
    ...
    for channel in cam_efficiency.keys():
        ...
        def radiation_integration(temperature, emissivity_data, cam_efficiency_data):
            def radiation(wl):
                return black_body_radiation(temperature, wl) * emissivity(temperature, wl, emissivity_data, melt_temperature, emissivity_liquid, emissivity_set) * camera_efficiency(wl, cam_efficiency_data)
    ...
    emi_field = emissivity_map(t_field, interp_emissivity, melt_temperature, emissivity_liquid, emissivity_set)
...
def emissivity_map(t_field, emi_function, melt_temperature, emissivity_liquid, emissivity_set):
    row, colum = t_field.shape
    emi_field = np.zeros(t_field.shape)
    wavelength = np.arange(500 * (10**-9), 901 * (10**-9))
    for i in range(row):
        for j in range(colum):
            emi_field[i, j] = np.average([emissivity(t_field[i, j], wl, emi_function, melt_temperature, emissivity_liquid, emissivity_set) for wl in wavelength])
    return emi_field
