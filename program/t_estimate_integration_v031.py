...
def t_estimate_integration():
    ...
    result_dir = 'result_v031_gray'
    result_dir = os.path.join('results', result_dir)
    data_temperature = '3501'
    emissivity_set = '1'
    ...
    target = intensity[:, 20:25, 20:25]
    t_target = t_ref[20:25, 20:25]
...
def compare(original_data, result_dir):
    ...
    t_bias = (t_target[20:25, 20:25] - t_cal) / t_target[20:25, 20:25]
    emi_bias = (emi_target[20:25, 20:25] - emi_cal) / emi_target[20:25, 20:25]
...
def emissivity_average_cal(a):
    wl0 = 500
    wl1 = 1000
    wavelength = np.arange(wl0, wl1) * 10**(-9)
    emissivity = np.average([emissivity_model(wl, a) for wl in wavelength])
    return emissivity
...
def integration(wl, f_array, qe_array, a, t):
    wl0 = 0.5 * 10 ** (-6)
    wl1 = 1 * 10 ** (-6)
    ...
    result = f * emissivity_model(wl, a) * qe * black_body_radiation(t, wl) * 200
    return result


def emissivity_model(wl, a):
...
def process_itg(intensity_array, qe_array, tr_array):
    ...
    def integration_solve(qe, a, t):
        result_f = []
        for i in range(8):
            funct = quad(integration, wl0, wl1, args=(tr_array, qe[i], a, t), epsabs = 1e-2, limit = 5)[0]
    ...
    with warnings.catch_warnings():
        ...
        popt, cov = curve_fit(integration_solve, qe_array, intensity_array, bounds=((0, 500), (1, 4000)), maxfev= 100000)
